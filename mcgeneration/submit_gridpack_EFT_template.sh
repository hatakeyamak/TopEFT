...
echo "Running in ${job_opt} [screen, interactive, etc] mode"
...
for ISETUPTAG in `seq 0 ${NSETUPTAG}`; do
	...
	./gridpack_generation.sh ${SETUP} addons/cards/${SETUP} local ALL ${scram_arch} ${cmssw_version}
    fi
